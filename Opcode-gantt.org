...
(let (a)
(setq a 0)
(org-map-entries (buffer-substring (point) (line-end-position)) "Effort=.*")
  ...
  (save-excursion
    (let (sum)
      ...
      (setq fibo (org-entry-get (point) "Effort"))
      (when fibo (setq sum (+ sum (string-to-number fibo)))
      )))
 sum
)
...
(save-excursion
(let (efforts)
(org-map-entries (lambda () (setq efforts (point))) "Effort=.*")
efforts
))
