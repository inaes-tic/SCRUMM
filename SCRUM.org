...
*** STARTED diseño: 
ahora es prioridad terminar los logos. recién hoy me pude poner a probar
colores. Pienso que el miércoles quizás sea un buen día para dedicarme a
conocer el inkscape, tendría mucho más sentido poder diseñar todo desde ahí.
para mí que el miércoles podríamos cerrar los logos y ver en qué estado está
el diseño de la web..
...
espera para suministrar los frames en tiempo real. Hay todavía algunos
escenarios donde al intentar cerrar melt algún thread queda bloqueado y no
cierra. Todavía no probé hacer que melted escriba su salida a memoria
compartida para leer desde varias fuentes. Hay que hacer pruebas con video
FullHD ya que parece que mi máquina no se lo banca. Se puede investigar
cómo hacer streaming desde el browser para agregar el preview a caspa.

*** PlayoutView:

Copio acá la lista de pendientes que escribimos en la otra cadena ya que el
último tiempo no hubo avances considerables.

**** TODO Buscar una solución para problemas de performance.
**** TODO Corregir un par de known-bugs.
**** TODO Mejorar la visualización y agregar unos datos que son necesarios.
**** TODO Agregar al comportamiento de drag and drop un método de "push down".
