...
***** Evitar modificar pl que tiene occurrences
***** Permitir duplicar pl para que no tenga que cargar todo de nuevo
**** Actualizar el mongo driver (Mosto)
**** Revisar
*** Mover al servidor el read de backbone.io
**** Adri lo sube, lo cerramos acá
*** DONE Backbone Model->get no busca en DB
**** Lo resuelve fetch related
*** BUG Backbone.io save->create
**** Probar qué pasa mandando CREATE de un elemento que ya está en la DB
*** BUG Save no siempre guarda (?)
*** BUG Cambios en memoria #148
**** No permitir cambiar de vista sin guardar (usar memento para rollback)
...
**** Averiguar que pasa al hacer SAVE con la lista filtrada por Knockback
...
*** STARTED BUG en Wrapper de melted
** alejo
